(function () {
  ...
  const logoMain = d3.select(".logo");
  const logoMenu = d3.select(".logo-menu");
  const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      logoMenu.classed("logo-menu-visible", !entry.isIntersecting);
    });
  });

  observer.observe(logoMain.node());
  ...
  const digestQuestions = function () {
    ...
    const questions = d3
      ...
      .style(
        ...
        "repeat(" + questionData.length + ", 1fr)"
      )
      .selectAll(".question")
      .data(questionData)
      .join(
